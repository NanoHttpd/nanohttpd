...
public abstract class NanoHTTPD {
	 ...
	 * 
	 * Pass a .jks resource filepath with your certificate and passphrase
	 */
	public static SSLServerSocketFactory makeSSLSocketFactory(String keyAndTrustStoreClasspathPath, char[] passphrase) throws IOException {
		InputStream keystoreStream = NanoHTTPD.class.getResourceAsStream(keyAndTrustStoreClasspathPath);
		return makeSSLSocketFactory(keystoreStream, passphrase);
	 ...
	 * Creates an SSLSocketFactory for HTTPS.
	 * 
	 * Pass a .bks resource with your certificate and passphrase
	 */
	 ...
	 * @return HTTP response, see class Response for details
	 */
	public abstract void serveForCallBack(String uri, Method method, Map<String, String> headers, Map<String, String> parms,
			Map<String, String> files);

	/**
	 * Called from HTTPSession if useCallback is true
	 * 
	 * Allows for call back
	 * @param session
	 * @param method
	 * @param outputStream
	 * @return
	 */
	public void serve(HTTPSession session,Method method,OutputStream outputStream)
	{
		this.method = method;
		...
		try {
			...
			sendCallBackResponse( new Response(Response.Status.INTERNAL_ERROR, MIME_PLAINTEXT, "SERVER INTERNAL ERROR: IOException: " + ioe.getMessage()));
		} catch (ResponseException re) {
			sendCallBackResponse( new Response(re.getStatus(), MIME_PLAINTEXT, re.getMessage()));
		}
		...
		serveForCallBack(uri, sessionMethod, headers, parms, files);
	}

	/**
	 * Sends response for callback process
	 * @throws ResponseException
	 */
	public void sendCallBackResponse(Response response)
	{
		if (response == null) {
			Response r = new Response(Response.Status.INTERNAL_ERROR, MIME_PLAINTEXT, "SERVER INTERNAL ERROR: Serve() returned a null response.");
			r.send(outputStream);
			safeClose(outputStream);
		} else {
	...
	protected class HTTPSession {
		...
		public void execute() throws IOException {
			try {
				...
				// Ok, now do the serve()
				if(useCallBack()){
					serve(this,method,outputStream);                	
				} else {
	...
	public final Boolean useCallBack(){
		return useCallback;
	}
}
